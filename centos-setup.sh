...
configure_shell() {
  # Installe un invite de commande personnalisé et quelques alias utiles
  ...
  # Les utilisateurs existants voudront peut etre l'utiliser
  if [ ! -z "${USERS}" ]
  then
    for USER in ${USERS}
    do
  ...
  # Ajoute un tas d'options systeme pratiques pour Vim
  echo 'Configuring Vim.'
  cat ${CWD}/${VERSION}/vim/vimrc > /etc/vimrc
  # Configure l'Anglais comme langage du systeme
...
configure_repos() {
  # Active les depots [base],[updates] et [extra] avec une priorite a 1
  ...
  # Active le repertoire [epel] avec une priorite a 10.
  ...
  # Configure les depots [elrepo] et [elrepo-kernel] sans les activer
  ...
  # Active le repertoire [lynis] avec une priorite a 5.
...
configure_logs() {
  # L'administrateur peut acceder aux journaux systeme
  if [ ! -z "${ADMIN}" ]
  then
...
strip_system() {
  # Supprime tous les paquets qui ne font pas partis du systeme de base ameliore
...
# S'assure que le script est execute avec les privileges de super utilisateur
if [[ "${UID}" -ne 0 ]]
then
